...
const stats=[["30","nhân viên dùng miễn phí",GREEN],["9","phân hệ nghiệp vụ",INDIGO],["1","nền tảng nối liền",PURPLE]];
...
const suite=[
 ["Hồ sơ & bản địa hóa VN","CCCD, MST, BHXH, người phụ thuộc, địa giới 2025, chứng chỉ",GREEN,"Miễn phí"],
 ["Tính lương Việt Nam","Gross→net, phiếu lương PDF, file NH, bảng kê",INDIGO],
 ["Chấm công ca kíp & tăng ca","3 ca 4 kíp, 150/200/300%, ca đêm",INDIGO],
 ["Máy chấm công & lương SP","ZKTeco/Hikvision, lương sản phẩm/khoán",INDIGO],
 ["Kê khai điện tử","Thuế TNCN 05/KK, BHXH D02-LT → HTKK",INDIGO],
 ["Cổng nhân viên (ESS)","Phiếu lương, xin nghỉ, tra cứu — mobile",INDIGO],
 ["Đánh giá KPI / OKR","Xếp loại, mục tiêu & kết quả then chốt",PURPLE],
 ["Đào tạo & An toàn LĐ","LMS tự cấp chứng chỉ, khám SK, tai nạn",PURPLE],
 ["Kho tri thức + Trợ lý AI","Kho tài liệu + AI hỏi-đáp (RAG)",PURPLE],
];
const cx=[0.6,4.93,9.26], cyy=[1.5,3.5,5.5], cw=4.04, ch=1.78;
suite.forEach((m,i)=>{ const x=cx[i%3], y=cyy[Math.floor(i/3)]; card(s,x,y,cw,ch);
  s.addShape(R,{x:x,y:y,w:0.12,h:ch,fill:{color:m[2]},rectRadius:0.0,line:{type:"none"}});
  s.addText(m[0],{x:x+0.28,y:y+0.16,w:cw-0.5,h:0.55,fontSize:13.5,bold:true,color:INK,fontFace:"Calibri",valign:"top",margin:0});
  s.addText(m[1],{x:x+0.28,y:y+0.74,w:cw-0.5,h:ch-0.85,fontSize:10.5,color:SLATE,fontFace:"Calibri",valign:"top",margin:0});
  if(m[3]) pill(s,x+cw-1.35,y+0.18,1.15,0.3,m[3],GREENBG,GREEN);
});
...
const pro=[
 ["Tính lương Việt Nam","Gross→net BHXH + thuế TNCN, giảm trừ gia cảnh · phiếu lương PDF · file chuyển khoản · bảng kê thuế & BHXH"],
 ["Chấm công ca kíp & tăng ca","3 ca 4 kíp · tăng ca 150/200/300% + ca đêm (miễn thuế phần chênh) → nối thẳng vào lương"],
 ["Máy chấm công & lương sản phẩm","Nhập tệp máy vân tay/khuôn mặt → bảng công · lương sản phẩm/khoán theo công đoạn"],
 ["Kê khai điện tử","Tờ khai thuế TNCN (05/KK, 05-1/BK) & BHXH (D02-LT) · kết xuất cho HTKK / phần mềm BHXH"],
 ["Cổng nhân viên (ESS)","Tự xem & tải phiếu lương, xem chấm công, xin nghỉ online, tra cứu tri thức — mobile"],
];
pro.forEach((m,i)=> modRow(s,0.7,1.75+i*1.07,12.0,i+1,m[0],m[1],INDIGO));
...
const ds=[["9","phân hệ nghiệp vụ"],["50","nhân viên demo"],["100%","đúng luật VN"],["On-prem","hoặc cloud"]];
